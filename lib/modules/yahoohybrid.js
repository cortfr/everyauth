...
openidModule.submodule('yahoohybrid')
  ...
  .verifyAttributes(function (req,res) {
    ...
    this.relyingParty.verifyAssertion(req, function (err, userAttributes) {
      if(err) return p.fail([err, req, res]);
      oauth.getOAuthAccessToken(userAttributes['request_token'], undefined, function (err, oauthAccessToken, oauthAccessTokenSecret, extra_data) {
        if (err) return p.fail([err, req, res]);
        userAttributes['access_token'] = oauthAccessToken;
        userAttributes['access_token_secret'] = oauthAccessTokenSecret;
        userAttributes['oauth_session_handle'] = extra_data.oauth_session_handle;   //Used for Access Token Renewal
        userAttributes['xoauth_yahoo_guid'] = extra_data.xoauth_yahoo_guid;
        userAttributes['oauth_expires_in'] = extra_data.oauth_expires_in;
        userAttributes['oauth_authorization_expires_in'] = extra_data.oauth_authorization_expires_in;
